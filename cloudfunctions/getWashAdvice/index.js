...
const WEATHER_HOST = process.env.QWEATHER_API_HOST || process.env.QWEATHER_HOST || "";
...
async function resolveWeatherLocationId(key, cityId, city) {
  if (city && city.weatherId) return city.weatherId;
  if (/^\d{9}$/.test(String(cityId || ""))) return cityId;

  const location = (city && (city.area || city.name)) || cityId;
  ...
  for (const adm of admCandidates) {
    ...
    if (adm) params.push(`adm=${encodeURIComponent(adm)}`);
    const url = `https://${normalizeHost(WEATHER_HOST)}/geo/v2/city/lookup?${params.join("&")}`;
...
function calculateAdvice(city, daily) {
  ...
  return {
    ...
    forecast: normalized.map(({ raw, ...item }) => item)
  };
}

exports.main = async event => {
  const key = process.env.QWEATHER_API_KEY || process.env.QWEATHER_KEY || "";
  const cityId = event.cityId || (event.city && event.city.id);
  const city = event.city || { id: cityId, name: "当前城市" };
  ...
  const weatherLocationId = await resolveWeatherLocationId(key, cityId, city);
  const url = `https://${normalizeHost(WEATHER_HOST)}/v7/weather/7d?location=${encodeURIComponent(weatherLocationId)}`;
  const response = await requestJSON(url, key);
  ...
  return {
    success: true,
    data: calculateAdvice(city, response.daily || [])
  };
};
